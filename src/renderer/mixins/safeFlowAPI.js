...
*
*/
safeFlowAPI.prototype.four = async function (authNetwork, authBundle) {
  console.log('')
}
...
*
*/
safeFlowAPI.prototype.five = async function (authNetwork, authBundle) {
  console.log('')
}
...
*
*/
safeFlowAPI.prototype.six = async function (authNetwork, authBundle) {
  console.log('')
}
...
*
*/
safeFlowAPI.prototype.seven = async function (authNetwork, authBundle) {
  console.log('')
}
...
*
*/
safeFlowAPI.prototype.eight = async function (authNetwork, authBundle) {
  console.log('')
}
...
*
*/
safeFlowAPI.prototype.nine = async function (authNetwork, authBundle) {
  console.log('')
}
...
/*
    ...
    let mappedNetworkExpKbundles = await this.safeAPI.experimentKbundles('retreive')
    ...
    for (let inxp of uniqueNXP) {
      let cnrlNXP = this.safeAPI.cnrlLookup(inxp)
  ...
  async startKSetting () {
    let startKset = await this.safeAPI.startSettings('retreive', null)
    // set via store and then pick up in historyData
  ...
  liveNetworkExperiments () {
    let experimentList = this.safeAPI.cnrlExperimentIndex()
    this.$store.dispatch('actionNetworkExperimentList', experimentList)
  },
  async deviceContext () {
    const deviceFlag = 'device'
    let deviceList = await this.safeAPI.toolkitContext(deviceFlag)
  ...
  async datatypeContext () {
    ...
    let datatypeList = await this.safeAPI.toolkitContext(dataTypeFlag, livePeerDevices)
    this.$store.dispatch('actionSetDataTypes', datatypeList)
  },
  cnrlComputeIndex () {
    // call the CNRL api and get network science active
    let startScienceCompute = this.safeAPI.cnrlNetworkComputeIndex()
  ...
  timeNav (navT) {
    let navTimelist = []
    navTimelist = this.safeAPI.cnrlTimeIndex(navT)
    this.$store.dispatch('actionTIMEindex', navTimelist)
  },
  GETexperimentsList () {
    let nxpList = this.safeAPI.cnrlExperimentIndex()
    this.$store.dispatch('actionNXPindex', nxpList)
  },
  GETdatatypeList () {
    let dtList = this.safeAPI.cnrlNetworkDatatypeIndex()
  ...
  saveStartBundle (bund) {
    ...
    this.safeAPI.startSettings('save', bund)
  },
  removeStartBundle (bund) {
    // need up date startStatus Object
    console.log('remove')
    this.safeAPI.startSettings('remove', bund)
  },
  removeStartDashboard (bund) {
    // need up date startStatus Object
    console.log('removedash')
    this.safeAPI.startSettings('removedash', bund)
  },
  async SaveexperimentKbundles (mapEKb) {
    let saveStatus = await this.safeAPI.experimentKbundles('save', mapEKb)
    return saveStatus
  },
  async mappedKBLexp () {
    let lastestMappedLedger = await this.safeAPI.experimentKbundles('retreive')
  ...
  async learnStart (lBundle) {
    ...
    this.activevis = this.$store.getters.liveVis[0]
    // make the Entity
    await this.safeAPI.scienceEntities(lBundle)
    // this.learnListening()
    let entityGetter = await this.safeAPI.entityGetter(this.activeEntity, 'vis-sc-1')
    let entityGetterTable = await this.safeAPI.entityGetter(this.activeEntity, 'vis-sc-2')
  ...
  learnListening () {
    ...
    this.safeAPI.liveEManager.on('computation', function (cState) {
  ...
  async getAverages (eid) {
    ...
    let AvgDailyHolder = {}
    let currentAHR = await this.safeAPI.entityCurrentAverageHR(eid, 'none')
    let newARHR = await this.safeAPI.entityCurrentAverageHR(eid, 'cnrl-8356388727')
    // let newARHR = 55
